...
%          gamj(i,j) ;
           ...
           upan_wake(i,j) = upan(i,j) + vinf*cos(alp) + (gam_imp/(1))*((ypmat(i,j)-y_imp(nx))./((xpmat(i,j)-x_imp(1,nx)).^2+(ypmat(i,j)-y_imp(nx)).^2));
           
           
           % Calculate local velocity near imposed vortex : y component
           vpan_wake(i,j) = vpan(i,j) + vinf * sin(alp) -(gam_imp/(1))*((xpmat(i,j)-x_imp(nx))./((xpmat(i,j)-x_imp(nx)).^2+(ypmat(i,j)-y_imp(nx)).^2));
