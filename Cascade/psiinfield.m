...
%          gamj(i,j) ;
           ...
           upan(i,j) = upan(i,j) + vinf*cos(alp); %+ (gam_imp/(2*pi))*((ypmat(i,j)-y_imp)./((xpmat(i,j)-x_imp).^2+(ypmat(i,j)-y_imp).^2));%+...
               %(gam_shed/(2*pi))*((ypmat(i,j)-y_fixed)./((xpmat(i,j)-x_fixed).^2+(ypmat(i,j)-y_fixed).^2));%Vortex added
           
           vpan(i,j) = vpan(i,j) + vinf * sin(alp); %+(-gam_imp/(2*pi))*((xpmat(i,j)-x_imp)./((xpmat(i,j)-x_imp).^2+(ypmat(i,j)-y_imp).^2));%+...
               %(-gam_shed/(2*pi))*((xpmat(i,j)-x_fixed)./((xpmat(i,j)-x_fixed).^2+(ypmat(i,j)-y_fixed).^2)); %% added vortex
