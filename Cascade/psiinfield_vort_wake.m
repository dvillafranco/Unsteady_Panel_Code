...
for j = 1:numx
    for i = 1:numy
        A1 =  -(xpmat(i,j) - xaf2(1:end-1)).*cos(thetpan) - (ypmat(i,j) - ynew(1:end-1)).*sin(thetpan);
        B1 =  (xpmat(i,j) - xaf2(1:end-1)).^2 + (ypmat(i,j) - ynew(1:end-1)).^2 ;
        E1 =  (xpmat(i,j) - xaf2(1:end-1)).*sin(thetpan) - (ypmat(i,j) - ynew(1:end-1)).*cos(thetpan);
...
%                 
           ...
           vp = gamans(1:end-1)'./2/pi.* (  (xpmat(i,j) - xaf2(1:end-1)).*(ga) - cos(thetpan).*(gb) ) + ...
               (gamans(2:end)'-gamans(1:end-1)') ./2/pi./SJ.*  (  (xpmat(i,j) - xaf2(1:end-1)).*(gb) - cos(thetpan).*(gc));
          
           
           vpj =  gamans(1:end-1)'/2/pi.* (  (xpmat(i,j) - xaf2(1:end-1)).*(ga) - cos(thetpan).*(gb) ) + ...
               (-gamans(1:end-1)') ./2/pi./SJ.*  (  (xpmat(i,j) - xaf2(1:end-1)).*(gb) - cos(thetpan).*(gc));
           vpjp1 = (gamans(2:end)') ./2/pi./SJ.*  (  (xpmat(i,j) - xaf2(1:end-1)).*(gb) - cos(thetpan).*(gc));
...
%         
        ...
        [maxxaf2 locxmax] = max(xaf2);
[minxaf2 locxmin] = min(xaf2);
...
%              
                ...
                %  y - ymin = (ymax - ymin)/(xmax-xmin)*(x - xmin)
                        yonchord = (ynew(locxmax) - ynew(locxmin))/(xaf2(locxmax)-xaf2(locxmin))*(xpmat(i,j) - xaf2(locxmin)) + ynew(locxmin);
                       
                if ( xpmat(i,j) > minxaf2 & xpmat(i,j) < maxxaf2 & ypmat(i,j) < yonchord |  i < locxmin) 
