...
%          gamj(i,j) ;
          ...
          gamdown = sum( gamans(2:end-1)'.*SJ/2 + gamans(1:end-2)'.*SJ/2) - vinf*h*sin(alp);
           ...
           upan_body(i,j) = upan(i,j)  + delta/2/pi*  ((xpmat(i,j)+1)./bot44(i,j) - (xpmat(i,j)-1)./bot33(i,j) ) -  gup /2/pi*(ypmat(i,j)./bot44(i,j)) - gamdown/2/pi*(ypmat(i,j)./bot33(i,j))+...
                (gam_imp/(2*pi))*((ypmat(i,j)-y_imp(nx))./((xpmat(i,j)-x_imp(nx)).^2+(ypmat(i,j)-y_imp(nx)).^2))+...
               sum((gam_dim./(2*pi)).*((ypmat(i,j)-y_fixed)./((xpmat(i,j)-x_fixed).^2+(ypmat(i,j)-y_fixed).^2)));
           
           
           vpan_body(i,j) = vpan(i,j) + delta/2/pi*(  ypmat(i,j)./bot44(i,j) - ypmat(i,j)./bot33(i,j)) + gup/2/pi*( (xpmat(i,j)+1)./bot44(i,j) ) + gamdown/2/pi*((xpmat(i,j)-1)./bot33(i,j))-...
               (gam_imp/(2*pi))*((xpmat(i,j)-x_imp(nx))./((xpmat(i,j)-x_imp(nx)).^2+(ypmat(i,j)-y_imp(nx)).^2))...
      ...
      %%%%
               ...
               phiII_overS = E1/2 -ga.*(-2*A1.^2 + B1)./SJ/2 - E1.*A1.*gb./SJ/2 + SJ/2.*gc;
                ...
                philocnew(i,j) = -sum(real(ansphipan))   + delta/4/pi*(  log( (xpmat(i,j)+1)^2+ypmat(i,j)^2) - log( (xpmat(i,j)-1)^2 + ypmat(i,j)^2))  +...
                         gup/2/pi* tatanup  + gamdown/2/pi*tatandown -(gam_imp/(2*pi))*(atan((ypmat(i,j)-y_imp(nx))./(xpmat(i,j)-x_imp(nx))))+...
