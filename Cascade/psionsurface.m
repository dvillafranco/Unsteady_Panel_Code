...
%          gamj(i,j) ;
           ...
           upan_body(i,j) = upan1(i,j) + vinf*cos(alp) + (gam_imp/(1))*((ypmat(i,j)-y_imp(nx))./((xpmat(i,j)-x_imp(1,nx)).^2+(ypmat(i,j)-y_imp(nx)).^2))+...
               sum((gam_dim./(1)).*((ypmat(i,j)-y_fixed)./((xpmat(i,j)-x_fixed).^2+(ypmat(i,j)-y_fixed).^2)));
           
           vpan_body(i,j) = vpan1(i,j) + vinf *sin(alp)-(gam_imp/(1))*((xpmat(i,j)-x_imp(nx))./((xpmat(i,j)-x_imp(nx)).^2+(ypmat(i,j)-y_imp(nx)).^2))...
