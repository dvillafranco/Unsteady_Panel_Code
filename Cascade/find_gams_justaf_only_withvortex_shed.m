...
vor_phi_y = (gam_imp/(1))*((xnew(1:end-1)-x_imp(nx))./((xnew(1:end-1)-x_imp(nx)).^2+(ynew(1:end-1)-y_imp(nx)).^2));

% dphi/dx for imposed vortex
vor_phi_x = (-gam_imp/(1))*((ynew(1:end-1)-y_imp(nx))./((xnew(1:end-1)-x_imp(nx)).^2+(ynew(1:end-1)-y_imp(nx)).^2));


% dphi/dy for shedded vortex
vor_phi_yshed = ((1/(1))*((xnew(1:end-1)-x_fixed(1))./((xnew(1:end-1)-x_fixed(1)).^2+(ynew(1:end-1)-y_fixed(1)).^2))).*cos(thetpan);

% dphi/dx for shedded vortex
vor_phi_xshed = ((1/(1))*((ynew(1:end-1)-y_fixed(1))./((xnew(1:end-1)-x_fixed(1)).^2+(ynew(1:end-1)-y_fixed(1)).^2))).*sin(thetpan);
