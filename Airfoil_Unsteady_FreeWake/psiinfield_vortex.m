...
%          gamj(i,j) ;
           ...
           upan_imp(i,j) = upan(i,j) + vinf*cos(alp) + ...
               sum((gam_dim./(2*pi)).*((ypmat(i,j)-y_fixed(1:end-1))./((xpmat(i,j)-x_fixed(1:end-1)).^2+(ypmat(i,j)-y_fixed(1:end-1)).^2)));
           
           
           % Calculate local velocity near imposed vortex : y component
           vpan_imp(i,j) = vpan(i,j) + vinf * sin(alp) +...
               sum((-gam_dim/(2*pi)).*((xpmat(i,j)-x_fixed(1:end-1))./((xpmat(i,j)-x_fixed(1:end-1)).^2+(ypmat(i,j)-y_fixed(1:end-1)).^2)));
