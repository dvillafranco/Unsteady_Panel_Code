...
% gup = vinf*h*sin(alp);
r_dis(nx) = sqrt((-0.5 - x_imp(nx)).^2 + (0 - y_imp(nx)).^2);

alp_induced(nx) = atan(gam_imp/(2*pi*(-0.5 - r_dis(nx))));
ADD_TEST = sin(thetpan-7.957*10^-4)';
ADD = sin(thetpan)' - ADD_TEST;


if nx == 1
    rhs1 =  vinf*sin(thetpan - alp_induced(nx)) ;      % Add vortex equations here ??
    %rhs1 = vinf*sin(thetpan- alp) + ADD';
else
    rhs1 = vinf*sin(thetpan - alp_induced(nx))  -  sum(gam_wake,2)'; %vormat(:,n-1)';
end
