...
% Take xp and yp to be the current position of the imposed vortex
%xp =  [-4.6 : .03 : 2.0 ];
 %yp = [-.4 :.03 : .4];
...
%          gamj(i,j) ;
           ...
           %upan(i,j) = upan(i,j) + vinf*cos(alp) + (gam_imp/(1))*((ypmat(i,j)-y_imp(nx))./((xpmat(i,j)-x_imp(1,nx)).^2+(ypmat(i,j)-y_imp(nx)).^2))+...
               %sum((gam_dim./(1)).*((ypmat(i,j)-y_fixed)./((xpmat(i,j)-x_fixed(1:end-1)).^2+(ypmat(i,j)-y_fixed).^2)));%Vortex added
           
           %vpan(i,j) = vpan(i,j) + vinf * sin(alp)-(gam_imp/(1))*((xpmat(i,j)-x_imp(nx))./((xpmat(i,j)-x_imp(nx)).^2+(ypmat(i,j)-y_imp(nx)).^2))+...
               %sum((-gam_dim/(1)).*((xpmat(i,j)-x_fixed(1:end-1))./((xpmat(i,j)-x_fixed(1:end-1)).^2+(ypmat(i,j)-y_fixed).^2))); %% added vortex
               
           if length(x_fixed) == 1
           % Calculated local velocity near imposed vortex : x component
           upan_wake(i,j) = upan(i,j) + vinf*cos(alp) + (gam_imp/(2*pi))*((ypmat(i,j)-y_imp(nx))./((xpmat(i,j)-x_imp(1,nx)).^2+(ypmat(i,j)-y_imp(nx)).^2));
           
           
           % Calculate local velocity near imposed vortex : y component
           vpan_wake(i,j) = vpan(i,j) + vinf * sin(alp) -(gam_imp/(2*pi))*((xpmat(i,j)-x_imp(nx))./((xpmat(i,j)-x_imp(nx)).^2+(ypmat(i,j)-y_imp(nx)).^2));
           ...
           upan_wake(i,j) = upan(i,j) + vinf*cos(alp) + (gam_imp/(2*pi))*((ypmat(i,j)-y_imp(nx))./((xpmat(i,j)-x_imp(1,nx)).^2+(ypmat(i,j)-y_imp(nx)).^2));
               ...
               wake_sum_x = 0;
               for ww = setdiff(1:length(x_fixed),nqr)
                  
                  wake_sum_x = wake_sum_x + (gam_dim(ww)./(2*pi)).*((ypmat(i,j)-y_fixed(ww))./((xpmat(i,j)-x_fixed(ww).^2+(ypmat(i,j)-y_fixed(ww)).^2)));
               end
           ...
           vpan_wake(i,j) = vpan(i,j) + vinf * sin(alp) -(gam_imp/(2*pi))*((xpmat(i,j)-x_imp(nx))./((xpmat(i,j)-x_imp(nx)).^2+(ypmat(i,j)-y_imp(nx)).^2));
               ...
               wake_sum_y = 0;
               for wy = setdiff(1:length(x_fixed),nqr)
    
                 wake_sum_y = wake_sum_y+ (-gam_dim(wy)/(2*pi)).*((xpmat(i,j)-x_fixed(wy))./((xpmat(i,j)-x_fixed(wy)).^2+(ypmat(i,j)-y_fixed(wy)).^2));
               end
...
%            
%            
%            % Calculated local velocity for wake: x and y component
%            for ww = 1:length(gam_xim)
%                upan_wake(ww) = upan(i,j) + vinf*cos(alp) + (gam_imp/(1))*((ypmat(i,j)-y_imp(nx))./((xpmat(i,j)-x_imp(1,nx)).^2+(ypmat(i,j)-y_imp(nx)).^2));
%                
%                vpan_wake(ww) = vpan(i,j) + vinf * sin(alp)-(gam_imp/(1))*((xpmat(i,j)-x_imp(nx))./((xpmat(i,j)-x_imp(nx)).^2+(ypmat(i,j)-y_imp(nx)).^2));
%                
%            end
%            
...
% vorposx = find(abs(xp-x_imp(nx))<0.02,1);
% vorposy = find(abs(yp-y_imp(nx))<0.02,1);
% 
% x_fixed(n) = x_fixed(n)+upan_imp*dt;
% y_imp(nx+1) = y_imp(nx)+vpan_imp*dt;

wake_convect_x(nqr) = upan_wake(i,j);
wake_convect_y(nqr) = vpan_wake(i,j);
