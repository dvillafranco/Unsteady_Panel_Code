...
%          gamj(i,j) ;
           ...
           upan_imp(i,j) = upan(i,j) + vinf*cos(alp) + ...
               sum((gam_dim./(1)).*((ypmat(i,j)-y_fixed)./((xpmat(i,j)-x_fixed(1:end-1)).^2+(ypmat(i,j)-y_fixed).^2)));
           
           
           % Calculate local velocity near imposed vortex : y component
           vpan_imp(i,j) = vpan(i,j) + vinf * sin(alp) +...
               sum((-gam_dim/(1)).*((xpmat(i,j)-x_fixed(1:end-1))./((xpmat(i,j)-x_fixed(1:end-1)).^2+(ypmat(i,j)-y_fixed).^2)));
...
% 
% x_imp(nx+1) = x_imp(nx)+upan_imp(vorposy,vorposx)*dt;
% y_imp(nx+1) = y_imp(nx);%+vpan_imp(vorposy,vorposx)*dt;
