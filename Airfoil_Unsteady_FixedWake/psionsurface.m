...
%          gamj(i,j) ;
           ...
           upan(i,j) = upan1(i,j) + vinf*cos(alp) + (gam_imp/(1))*((ypmat(i,j)-y_imp(nx))./((xpmat(i,j)-x_imp(1,nx)).^2+(ypmat(i,j)-y_imp(nx)).^2))+...
               sum((gam_dim./(1)).*((ypmat(i,j)-y_fixed)./((xpmat(i,j)-x_fixed).^2+(ypmat(i,j)-y_fixed).^2)));
           
           vpan(i,j) = vpan1(i,j) + vinf *sin(alp)-(gam_imp/(1))*((xpmat(i,j)-x_imp(nx))./((xpmat(i,j)-x_imp(nx)).^2+(ypmat(i,j)-y_imp(nx)).^2))...
...
for aa = 1:length(gam_dim)
    gam_wake(:,aa) = (1*gam_dim(aa).*((1/(1))*((xnew(1:end-1)-x_fixed(aa+1))./((xnew(1:end-1)-x_fixed(aa+1)).^2+(ynew(1:end-1)-y_fixed).^2))).*cos(thetpan)+...
    (1*gam_dim(aa)*((1/(1))*((ynew(1:end-1)-y_fixed)./((xnew(1:end-1)-x_fixed(aa+1)).^2+(ynew(1:end-1)-y_fixed).^2))).*sin(thetpan)));
end
